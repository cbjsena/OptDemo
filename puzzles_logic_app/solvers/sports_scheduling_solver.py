...
class OrtoolsComplexSolver(BaseOrtoolsSportsSolver):
    ...
    def _extract_results(self, solver):
        ...
        for s in range(self.num_slots):
            ...
            for h in range(self.num_teams):
                for a in range(self.num_teams):
                    if h != a and self.plays.get((s, h, a)) and solver.Value(self.plays[(s, h, a)]) > 0.5:
...
class GurobiSimpleSolver(BaseGurobiSportsSolver):
    ...
    def solve(self):
        ...
        logger.info(f"Running GurobiSimpleSolver for {self.num_teams_original} teams.")

        self._create_variables()
        self._add_common_constraints()

        # --- 여기에 run_sports_scheduling_optimizer_gurobi1의 목표 함수 및 결과 추출 로직 구현 ---

        self.model.setParam('TimeLimit', settings.GUROBI_TIME_LIMIT)
        self.model.optimize()
...
class GurobiComplexSolver(BaseGurobiSportsSolver):
    ...
    def solve(self):
        start_time = time.time()
        logger.info(f"Running GurobiComplexSolver for {self.num_teams_original} teams.")

        self._create_variables()
        # --- 여기에 run_sports_scheduling_optimizer_gurobi2의 추가 변수, 제약, 목표 함수, 결과 추출 로직 구현 ---
        # 예: self.is_at_loc = self.model.addVars(...)
        self._add_common_constraints()
        # 예: self._add_location_constraints()
